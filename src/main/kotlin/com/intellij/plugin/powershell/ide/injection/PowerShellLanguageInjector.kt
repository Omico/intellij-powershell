...
class PowerShellLanguageInjector : MultiHostInjector {
    ...
    private val injectionSupport: PowerShellLanguageInjectionSupport? by lazy {
        ArrayList(InjectorUtils.getActiveInjectionSupports()).filterIsInstance(PowerShellLanguageInjectionSupport::class.java)
            .firstOrNull()
    }
    ...
    private fun splitLiteralToInjectionParts(
        baseInjection: BaseInjection,
        context: PowerShellStringLiteralExpression,
    ): PowerShellInjectionParts {
        ...
        val stringContentSet =
            TokenSet.create(PowerShellTypes.EXPANDABLE_STRING_PART, PowerShellTypes.EXPANDABLE_HERE_STRING_PART)
        val expressionContentSet =
            TokenSet.create(PowerShellTypes.SUB_EXPRESSION, PowerShellTypes.TARGET_VARIABLE_EXPRESSION)
        ...
        while (i < len) {
            ...
            val part: Any = when {
                expressionContentSet.contains(c.elementType) -> {
                    ...
                    val partSize =
                        childNodes.subList(i, len).asSequence().takeWhile { stringContentSet.contains(it.elementType) }
                            .count()
                    i += partSize - 1
                    childNodes[i]
                }
            ...
            if (part is ASTNode) {
                ranges.add(
                    createInjectionRange(
                        ...
                        TextRange.create(
                            partOffsetInParent,
                            (part.startOffset - part.treeParent.startOffset) + part.textLength,
                        ),
                        prefix,
                        suffix,
                    ),
                ...
                ranges.add(
                    createInjectionRange(
                        context,
                        baseInjection,
                        TextRange.from(partOffsetInParent, 0),
                        prefix,
                        suffix,
                    ),
                )
        ...
        if (lastChild != null && prefix.isNotEmpty()) {
            ...
            createInjectionRange(
                context,
                baseInjection,
                TextRange.from((lastChild.startOffset - lastChild.treeParent.startOffset) + lastChild.textLength, 0),
                prefix,
                baseInjection.suffix,
            )
        }
    ...
    private fun createInjectionRange(
        literal: PowerShellStringLiteralExpression,
    ...
    class PowerShellInjectionParts(
        val ranges: MutableList<Trinity<PsiLanguageInjectionHost, InjectedLanguage, TextRange>>,
        val isUnparsable: Boolean = false,
    )
}
