...
import com.intellij.plugin.powershell.psi.PowerShellUnaryExpression
...
import com.intellij.plugin.powershell.psi.isFunctionDeclarationContext
import com.intellij.plugin.powershell.psi.isLabelContext
import com.intellij.plugin.powershell.psi.isPathExpressionContext
import com.intellij.plugin.powershell.psi.isTargetVariableContext
import com.intellij.psi.TokenType
import com.intellij.psi.tree.IElementType

internal fun canAddSpaceBefore(node: ASTNode): Boolean =
    !isLabelContext(node) && !isTargetVariableContext(node) && !isPathExpressionContext(node) && !isFunctionDeclarationContext(
        node,
    )

internal fun isIdentifier(node: ASTNode) =
    (node.elementType != PowerShellTypes.REFERENCE_IDENTIFIER || node.elementType != PowerShellTypes.IDENTIFIER)
...
internal fun isRedirectionOperator(type: IElementType) =
    PowerShellTypes.OP_FR === type || PowerShellTypes.OP_MR === type
...
internal fun isFollowedByAttribute(childNode: ASTNode) = findSiblingSkipping(
    childNode,
    arrayOf(PowerShellTypes.NLS, TokenType.WHITE_SPACE),
    false,
)?.elementType === PowerShellTypes.ATTRIBUTE
...
internal fun isBlockParameter(childNode: ASTNode) =
    isBlockParameterClauseContext(childNode) && canBeParameter(childNode)
