...
import com.intellij.plugin.powershell.psi.PowerShellAttribute
...
import com.intellij.plugin.powershell.psi.PowerShellTypes.ATTRIBUTE
import com.intellij.plugin.powershell.psi.PowerShellTypes.BLOCK_BODY
import com.intellij.plugin.powershell.psi.PowerShellTypes.CAST_EXPRESSION
import com.intellij.plugin.powershell.psi.PowerShellTypes.CATCH_CLAUSE
import com.intellij.plugin.powershell.psi.PowerShellTypes.CLASS_DECLARATION_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.COLON
import com.intellij.plugin.powershell.psi.PowerShellTypes.COMMA
import com.intellij.plugin.powershell.psi.PowerShellTypes.CONSTRUCTOR_DECLARATION_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.DATA_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.DO_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.DS
import com.intellij.plugin.powershell.psi.PowerShellTypes.ELSEIF_CLAUSE
import com.intellij.plugin.powershell.psi.PowerShellTypes.ELSE_CLAUSE
import com.intellij.plugin.powershell.psi.PowerShellTypes.ENUM_DECLARATION_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.EXPANDABLE_HERE_STRING_END
import com.intellij.plugin.powershell.psi.PowerShellTypes.FINALLY_CLAUSE
...
import com.intellij.plugin.powershell.psi.PowerShellTypes.FOREACH_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.FOR_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.FUNCTION_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.IDENTIFIER
import com.intellij.plugin.powershell.psi.PowerShellTypes.IF
import com.intellij.plugin.powershell.psi.PowerShellTypes.IF_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.INLINESCRIPT_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.INVOCATION_EXPRESSION
import com.intellij.plugin.powershell.psi.PowerShellTypes.LCURLY
import com.intellij.plugin.powershell.psi.PowerShellTypes.LP
import com.intellij.plugin.powershell.psi.PowerShellTypes.METHOD_DECLARATION_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.NLS
import com.intellij.plugin.powershell.psi.PowerShellTypes.PARALLEL_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.PARAMETER_CLAUSE
import com.intellij.plugin.powershell.psi.PowerShellTypes.PARENTHESIZED_ARGUMENT_LIST
import com.intellij.plugin.powershell.psi.PowerShellTypes.RCURLY
import com.intellij.plugin.powershell.psi.PowerShellTypes.RP
import com.intellij.plugin.powershell.psi.PowerShellTypes.SEMI
import com.intellij.plugin.powershell.psi.PowerShellTypes.SEQUENCE_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.SIMPLE_ID
import com.intellij.plugin.powershell.psi.PowerShellTypes.SQBR_L
import com.intellij.plugin.powershell.psi.PowerShellTypes.SQBR_R
import com.intellij.plugin.powershell.psi.PowerShellTypes.SWITCH_CLAUSE_BLOCK
import com.intellij.plugin.powershell.psi.PowerShellTypes.SWITCH_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.TRAP_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.TRY_STATEMENT
import com.intellij.plugin.powershell.psi.PowerShellTypes.TYPE_LITERAL_EXPRESSION
import com.intellij.plugin.powershell.psi.PowerShellTypes.WHILE
import com.intellij.plugin.powershell.psi.PowerShellTypes.WHILE_STATEMENT
import com.intellij.plugin.powershell.psi.findSiblingSkipping
import com.intellij.plugin.powershell.psi.findSiblingSkippingWS
...
import com.intellij.psi.codeStyle.CommonCodeStyleSettings.NEXT_LINE
import com.intellij.psi.codeStyle.CommonCodeStyleSettings.NEXT_LINE_IF_WRAPPED
import com.intellij.psi.codeStyle.CommonCodeStyleSettings.NEXT_LINE_SHIFTED
import com.intellij.psi.codeStyle.CommonCodeStyleSettings.NEXT_LINE_SHIFTED2
 ...
 * Andrey 05/08/17.
 */
open class PowerShellBlockImpl(
    node: ASTNode,
    wrap: Wrap?,
    alignment: Alignment?,
    indent: Indent?,
    ...
    psStyleSettings: PowerShellCodeStyleSettings,
    ...
    constructor(file: PsiFile, setting: CodeStyleSettings) : this(
        file.node, Wrap.createWrap(WrapType.NONE, false), Alignment.createAlignment(),
    ...
    override fun getSpacing(child1: Block?, child2: Block): Spacing? {
        return if (child2 is PowerShellCodeBlock) {
            mySpacingProcessor.getSpacing(
                child1 as? PowerShellCodeBlock?,
                child2,
            )
        } else {
            null
    ...
    override fun buildChildren(): MutableList<Block> {
        ...
        while (childNode != null) {
            ...
            val childBlock: PowerShellCodeBlock = if (childNode.elementType === INVOCATION_EXPRESSION) {
                createCallExpressionBlock(
                    childNode,
                    null /*createWrap(childNode)*/,
                    createAlignment(childNode),
                    createIndent(childNode),
                )
            } else {
                PowerShellBlockImpl(
                    childNode,
                    createWrap(childNode),
                    createAlignment(childNode),
                    createIndent(childNode),
                    myIndentSize,
                    myCommonSettings,
    ...
    private fun createCallExpressionBlock(
        node: ASTNode,
        wrap: Wrap?,
        alignment: Alignment?,
        indent: Indent?,
        ...
        val callChainBlock =
            ChainMethodCallBlockBuilder(alignment, wrap, indent, myCommonSettings, myPSSettings).build(nodes)
...
//    if (isInvocationExpressionQualifier(node) && myCommonSettings.ALIGN_MULTILINE_CHAINED_METHODS) return myBaseAlignment
        ...
        if (myNode.elementType === PARENTHESIZED_ARGUMENT_LIST &&
            ...
            myCommonSettings.CALL_PARAMETERS_LPAREN_ON_NEXT_LINE
        ) {
            return myBaseAlignment
        }
        ...
        if (myNode.elementType === PARAMETER_CLAUSE &&
            ...
            myCommonSettings.METHOD_PARAMETERS_LPAREN_ON_NEXT_LINE
        ) {
            return myBaseAlignment
        }
    ...
    private fun isLhsOrRhsBinaryExpression(node: ASTNode) =
        isBinaryExpressionContext(node) && node.psi is PowerShellExpression
    ...
    private fun createWrap(childNode: ASTNode): Wrap {
        if ((isFunctionParameter(childNode) || isBlockParameter(childNode)) && findSiblingSkipping(
                childNode,
                arrayOf(NLS, WHITE_SPACE),
                false,
            )?.elementType != LP
        ) {
            val firstNode = isFirstNodeInParameter(childNode, true)
            val wrapValue =
                if (isFunctionParameter(childNode)) myCommonSettings.METHOD_PARAMETERS_WRAP else myPSSettings.BLOCK_PARAMETER_CLAUSE_WRAP
            val paramWrap = Wrap.createWrap(WrappingUtil.getWrapType(wrapValue), firstNode)
            if (firstNode) {
                if (isAttribute(childNode)) {
                    val attributeWrap =
                        Wrap.createWrap(WrappingUtil.getWrapType(myCommonSettings.PARAMETER_ANNOTATION_WRAP), true)
        ...
        if (isCallArgument(childNode) && findSiblingSkipping(
                childNode,
                arrayOf(NLS, WHITE_SPACE),
                false,
            )?.elementType != LP
            ...
            return Wrap.createWrap(
                WrappingUtil.getWrapType(myCommonSettings.CALL_PARAMETERS_WRAP),
                isFirstNodeInParameter(childNode, true),
            )
        }

        if (isForParameter(childNode)) {
            return Wrap.createWrap(
                WrappingUtil.getWrapType(myCommonSettings.FOR_STATEMENT_WRAP),
                isFirstNodeInForParameter(childNode),
            )
        }
...
class PowerShellSpacingProcessor(
    private val myCommonSettings: CommonCodeStyleSettings,
    private val myPowerShellSettings: PowerShellCodeStyleSettings,
) {
    ...
    internal fun getSpacing(child1: PowerShellCodeBlock?, child2: PowerShellCodeBlock): Spacing? {
        ...
        if (LCURLY === type1 || RCURLY === type2) {
            ...
            val isSimpleBlock =
                blockBodyToken === BLOCK_BODY && !containsNewLines(blockBody) || (blockBodyToken === LCURLY || blockBodyToken === RCURLY)
            if (isSimpleBlock) {
        ...
        if (SQBR_L === type1 || SQBR_R === type2) {
            if (isCastExpressionSuperContext(node1) || isCastExpressionSuperContext(node2)) {
                return simpleSpacing(
                    myCommonSettings.SPACE_WITHIN_CAST_PARENTHESES,
                )
            }

            return simpleSpacing(myCommonSettings.SPACE_WITHIN_BRACKETS)
        }
        if (type1 === TYPE_LITERAL_EXPRESSION && node1.treeParent?.elementType == CAST_EXPRESSION) {
            return simpleSpacing(
                myCommonSettings.SPACE_AFTER_TYPE_CAST,
            )
        }
        ...
        if (LP === type1) {
            if (isArgumentListContext(node1)) {
                ...
                val lfAfterLparen =
                    myCommonSettings.CALL_PARAMETERS_WRAP != DO_NOT_WRAP && myCommonSettings.CALL_PARAMETERS_LPAREN_ON_NEXT_LINE
                if (treeNext?.elementType === RP) {
                    val lfAfterRparen =
                        myCommonSettings.CALL_PARAMETERS_WRAP != DO_NOT_WRAP && myCommonSettings.CALL_PARAMETERS_RPAREN_ON_NEXT_LINE
                    return createSpacing(
                        myCommonSettings.SPACE_WITHIN_EMPTY_METHOD_CALL_PARENTHESES,
                        lfAfterLparen || lfAfterRparen,
                    )
                }
            ...
            if (isParameterClauseContext(node1) || isBlockParameterClauseContext(node1)) {
                ...
                val wrapValue =
                    if (methodParam) myCommonSettings.METHOD_PARAMETERS_WRAP else myPowerShellSettings.BLOCK_PARAMETER_CLAUSE_WRAP
                val lParenNextLine =
                    if (methodParam) myCommonSettings.METHOD_PARAMETERS_LPAREN_ON_NEXT_LINE else myPowerShellSettings.BLOCK_PARAMETERS_LPAREN_ON_NEXT_LINE
                val lfAfterLparen = wrapValue != DO_NOT_WRAP && lParenNextLine

                if (treeNext?.elementType === RP) {
                    val rParenNextLine =
                        if (methodParam) myCommonSettings.METHOD_PARAMETERS_RPAREN_ON_NEXT_LINE else myPowerShellSettings.BLOCK_PARAMETERS_RPAREN_ON_NEXT_LINE
                    val lfAfterRparen = wrapValue != DO_NOT_WRAP && rParenNextLine
                    return createSpacing(
                        myCommonSettings.SPACE_WITHIN_EMPTY_METHOD_PARENTHESES,
                        lfAfterLparen || lfAfterRparen,
                    )
                }
            ...
            if (isForClauseContext(node1)) {
                return createSpacing(
                    myCommonSettings.SPACE_WITHIN_FOR_PARENTHESES,
                    myCommonSettings.FOR_STATEMENT_LPAREN_ON_NEXT_LINE,
                )
            }
        ...
        if (RP === type2) {
            ...
            if (isForClauseContext(node2)) {
                return createSpacing(
                    myCommonSettings.SPACE_WITHIN_FOR_PARENTHESES,
                    myCommonSettings.FOR_STATEMENT_RPAREN_ON_NEXT_LINE,
                )
            }
        ...
        if (isMultiplicativeOperator(type1) || isMultiplicativeOperator(type2)) {
            if (!isIdentifier(node1.treeParent) && !isIdentifier(node2.treeParent)) {
                return simpleSpacing(
                    myCommonSettings.SPACE_AROUND_MULTIPLICATIVE_OPERATORS,
                )
            }
        }
        ...
        if (type2 === LCURLY) {
            if (isClassDeclarationContext(node2) || isEnumDeclarationContext(node2)) {
                ...
                return createBraceSpacing(
                    myCommonSettings.SPACE_BEFORE_CLASS_LBRACE,
                    myCommonSettings.CLASS_BRACE_STYLE,
                )
            }
            if (isCallableDeclarationContext(node2)) {
                ...
                return createBraceSpacing(
                    myCommonSettings.SPACE_BEFORE_METHOD_LBRACE,
                    myCommonSettings.METHOD_BRACE_STYLE,
                )
            }
    ...
    private fun createSpacing(
        addSpace: Boolean,
        ensureLineBreak: Boolean,
        keepLineBreaks: Boolean = myCommonSettings.KEEP_LINE_BREAKS,
        keepBlankLines: Int = keepBlankLines(),
    ): Spacing {
    ...
    private fun createBraceSpacing(
        addSingleSpace: Boolean,
        bracePlacementStyle: Int,
        dependantTextRange: TextRange? = null,
    ): Spacing {
        return when (bracePlacementStyle) {
            ...
            NEXT_LINE_IF_WRAPPED -> if (dependantTextRange != null) {
                dependentLFSpacing(
                    addSingleSpace,
                    dependantTextRange,
                )
            } else {
                simpleSpacing(addSingleSpace)
            }
        ...
        return when (bracePlacementStyle) {
            ...
            NEXT_LINE_IF_WRAPPED -> {
                if (dependantTextRange != null) {
                    ...
                    Spacing.createSpacing(minSpaces, maxSpaces, 0, myCommonSettings.KEEP_LINE_BREAKS, keepBlankLines())
                }
            }
