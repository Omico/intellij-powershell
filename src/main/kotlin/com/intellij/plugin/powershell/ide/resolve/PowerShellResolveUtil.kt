...
object PowerShellResolveUtil {
    ...
    fun collectChildrenDeclarations(
        context: PsiElement,
        result: HashSet<PowerShellComponent>,
        processor: PsiScopeProcessor,
        state: ResolveState,
        lastParent: PsiElement?,
    ) {
    ...
    fun processClassMembers(
        clazz: PowerShellClassDeclarationStatement,
        processor: PowerShellComponentScopeProcessor,
        state: ResolveState,
        lastParent: PsiElement?,
        place: PsiElement?,
    ) {
    ...
    fun processEnumMembers(
        enum: PowerShellEnumDeclarationStatement,
        processor: PowerShellComponentScopeProcessor,
        state: ResolveState,
        lastParent: PsiElement?,
        place: PsiElement?,
    ...
    fun processMembersForType(
        psType: PowerShellType,
        incompleteCode: Boolean,
        resolveProcessor: PowerShellMemberScopeProcessor,
    ): Boolean {
        val declarationsProvider = Processor<PowerShellMemberScopeProcessor> { processor ->
            psType.accept(object : PowerShellTypeVisitor<Boolean>() {
                ...
                override fun visitArrayClassType(o: PowerShellArrayClassType): Boolean {
                    val componentType =
                        o.getComponentType() // todo resolve to the actual array class (PS actually tries to resolve to component type in some cases)
    ...
    private fun processDeclarations(
        declarationsProvider: Processor<PowerShellMemberScopeProcessor>,
        incompleteCode: Boolean,
        resolveProcessor: PowerShellMemberScopeProcessor,
    ): Boolean {
    ...
    private fun processMembers(
        clazz: PowerShellTypeDeclaration,
        resolveProcessor: PowerShellMemberScopeProcessor,
    ): Boolean {
        ...
        if (baseClass != null) {
            ...
            if (baseDeclaration is PowerShellTypeDeclaration && processMembers(
                    baseDeclaration,
                    resolveProcessor,
                )
            ) {
