...
class PowerShellUsagesProvider : FindUsagesProvider {
    ...
    override fun getType(element: PsiElement): String {
        ...
        return componentType?.toString()?.lowercase(Locale.getDefault()) ?: "reference"
    }

    override fun getDescriptiveName(element: PsiElement): String {
        return when (element) {
            is PsiNamedElement ->
                PowerShellComponentType.typeOf(element)?.toString()
                    ?.lowercase(Locale.getDefault()) + " " + StringUtil.notNullize(element.name)
