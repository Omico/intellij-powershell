...
object PowerShellNameUtils {
    ...
    fun isValidName(name: String, element: PsiElement): Boolean = when {
        isBracedVariable(element) -> PowerShellTokenTypeSets.BRACED_VARIABLE_IDENTIFIERS.contains(
            getTokenType(
                name,
                _PowerShellLexer.VAR_BRACED,
            ),
        )
        isSimpleVariable(element) -> PowerShellTokenTypeSets.SIMPLE_VARIABLE_IDENTIFIERS.contains(
            getTokenType(
                name,
                _PowerShellLexer.VAR_SIMPLE,
            ),
        )
