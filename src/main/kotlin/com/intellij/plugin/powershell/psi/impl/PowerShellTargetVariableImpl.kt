...
import com.intellij.plugin.powershell.psi.PowerShellTypes.AT
import com.intellij.plugin.powershell.psi.PowerShellTypes.BRACED_VAR_START
 ...
 * Andrey 18/08/17.
 */
open class PowerShellTargetVariableImpl(node: ASTNode) :
    ...
    private fun findInitializer(resolved: PowerShellVariable): PowerShellPsiElement? {
        ...
        val assignment = PsiTreeUtil.getParentOfType(
            resolved,
            PowerShellAssignmentExpression::class.java,
            true,
    ...
    override fun resolve(): PowerShellComponent? {
        ...
        if (isInstanceThis()) {
            return PsiTreeUtil.findFirstContext(
                this,
                false,
            ) { c -> c is PowerShellTypeDeclaration } as? PowerShellComponent
        }
    ...
    override fun getRangeInElement(): TextRange =
        TextRange(getPrefixNode().textLength, node.textLength - (getSuffixNode()?.textLength ?: 0))
...
//  }
    ...
    private fun isLhsAssignmentTarget(): Boolean =
        context?.firstChild == this && context is PowerShellAssignmentExpression
}
