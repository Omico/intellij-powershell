...
abstract class PowerShellAbstractStringLiteralExpressionImpl(node: ASTNode) :
    PowerShellExpressionImpl(node),
    PsiLanguageInjectionHost,
    PowerShellStringLiteralExpression {

    override fun hasSubstitutions(): Boolean =
        isExpandable() && children.any { it is PowerShellVariable || it is PowerShellSubExpression }
    ...
    override fun isHereString(): Boolean =
        firstChild.node.elementType == PowerShellTypes.EXPANDABLE_HERE_STRING_START || firstChild.node.elementType == PowerShellTypes.VERBATIM_HERE_STRING

    override fun isExpandable(): Boolean =
        firstChild.node.elementType == PowerShellTypes.DQ_OPEN || firstChild.node.elementType == PowerShellTypes.EXPANDABLE_HERE_STRING_START

    override fun isValidHost(): Boolean = true

    override fun updateText(text: String): PsiLanguageInjectionHost =
        ElementManipulators.handleContentChange<PowerShellStringLiteralExpression>(this, text)

    override fun createLiteralTextEscaper(): LiteralTextEscaper<out PsiLanguageInjectionHost> =
        StringLiteralEscaper(this)

    class StringLiteralEscaper internal constructor(host: PowerShellStringLiteralExpression) :
        LiteralTextEscaper<PowerShellStringLiteralExpression>(host) {
