...
import com.intellij.plugin.powershell.psi.PowerShellCommandParameter
import com.intellij.plugin.powershell.psi.PowerShellIdentifier
import com.intellij.plugin.powershell.psi.PowerShellIncompleteDeclaration
import com.intellij.plugin.powershell.psi.PowerShellInvocationExpression
import com.intellij.plugin.powershell.psi.PowerShellLabel
...
class PowerShellAnnotator : Annotator {
    override fun annotate(e: PsiElement, holder: AnnotationHolder) {
        when {
            ...
            e is PowerShellIdentifier && PowerShellPsiImplUtil.isTypeDeclarationContext(e) -> createInfoAnnotation(
                holder,
                e,
                PowerShellSyntaxHighlighter.TYPE_NAME,
            )
            e is PowerShellReferenceTypeElement -> createInfoAnnotation(
                holder,
                e,
                PowerShellSyntaxHighlighter.TYPE_REFERENCE,
            )
            e is PowerShellTargetVariableExpression -> annotateVariable(
                holder,
                e,
                PowerShellSyntaxHighlighter.VARIABLE_NAME,
            )
            e is PowerShellMemberAccessExpression -> annotateMemberAccess(
                holder,
                e,
                PowerShellSyntaxHighlighter.PROPERTY_REFERENCE,
            )
            e is PowerShellInvocationExpression -> annotateMethodCallName(
                holder,
                e,
                PowerShellSyntaxHighlighter.METHOD_CALL,
            )
            e is PowerShellCallableDeclaration -> annotateFunctionName(
                holder,
                e,
                PowerShellSyntaxHighlighter.METHOD_DECLARATION,
            )
            e is PowerShellLabel -> createInfoAnnotation(holder, e.identifier, PowerShellSyntaxHighlighter.LABEL)
            e is PowerShellLabelReferenceExpression -> createInfoAnnotation(
                holder,
                e.identifier,
                PowerShellSyntaxHighlighter.LABEL,
            )
            e is PowerShellIncompleteDeclaration -> createErrorAnnotation(holder, e, "Incomplete declaration")
            shouldAnnotateAsParameter(e) -> createInfoAnnotation(
                holder,
                e,
                PowerShellSyntaxHighlighter.COMMAND_PARAMETER,
            )
        }
    }
    ...
    private fun annotateFunctionName(
        holder: AnnotationHolder,
        callable: PowerShellCallableDeclaration,
        attributes: TextAttributesKey,
    ) {
        createInfoAnnotation(holder, callable.nameIdentifier, attributes)
    }

    private fun annotateMethodCallName(
        holder: AnnotationHolder,
        call: PowerShellInvocationExpression,
        attributes: TextAttributesKey,
    ) {
    ...
    private fun annotateMemberAccess(
        holder: AnnotationHolder,
        expression: PowerShellMemberAccessExpression,
        attributes: TextAttributesKey,
    ) {
    ...
    private fun annotateVariable(
        holder: AnnotationHolder,
        variable: PowerShellTargetVariableExpression,
        attributes: TextAttributesKey,
    ) {
        ...
        if (!PowerShellTokenTypeSets.KEYWORDS.contains(varId.firstChild?.node?.elementType)) {
