...
class EditorEventManager(
    ...
    private val languageServerEndpoint: LanguageServerEndpoint,
) {
    ...
    private val changesParams = DidChangeTextDocumentParams(
        ...
        Collections.singletonList(TextDocumentContentChangeEvent()),
    )
            ...
            if (isOpen) {
                ...
                requestManager.didOpen(
                    DidOpenTextDocumentParams(
                        TextDocumentItem(
                            identifier.uri,
                            PowerShellLanguage.INSTANCE.id,
                            incVersion(),
                ...
                when (syncKind) {
                    ...
                    -> {
                        ...
                        val (endLine, endColumn) = if (oldText.isNotEmpty()) {
                            ...
                            Pair(startLine, startColumn) // if insert or no text change, the end position is the same
                        }
...
val DEFAULT_DID_CHANGE_CONFIGURATION_PARAMS =
    DidChangeConfigurationParams(PowerShellLanguageServerSettingsWrapper(LanguageServerSettings()))
...
data class LanguageServerSettings(
    val EnableProfileLoading: Boolean = true,
    val ScriptAnalysis: ScriptAnalysisSettings = ScriptAnalysisSettings(),
    val CodeFormatting: CodeFormattingSettings = CodeFormattingSettings(),
)
