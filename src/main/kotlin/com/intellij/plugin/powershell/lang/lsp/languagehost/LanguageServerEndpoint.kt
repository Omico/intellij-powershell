                        ...
                        try {
                            if (!connectedEditors.contains(uri)) {
                                ...
                                val syncKind: TextDocumentSyncKind? =
                                    if (syncOptions.isRight) syncOptions.right.change else syncOptions.left
    ...
    private fun getServerCapabilities(documentPath: String): ServerCapabilities? {
        ...
        try {
            ...
            initializeFuture?.get(
                if (capabilitiesAlreadyRequested) 0 else 9000 /*Timeout.INIT_TIMEOUT*/,
        ...
        return if (initializeResult != null) {
...
//      fileWriter = PrintWriter(dumpFile)
        ...
        val listener = object : VirtualFileListener {
            override fun contentsChanged(event: VirtualFileEvent) {
                ...
                if (isRemotePath(canonicalPath)) {
                    ...
                    ) // notify Editor Services to save remote file
                }
            }
        }
    ...
    private fun showPowerShellNotConfiguredNotification() {
        ...
        val content =
            MessagesBundle.message("vs.code.powershell.extension.configure.message", downloadLink, configureLink)
