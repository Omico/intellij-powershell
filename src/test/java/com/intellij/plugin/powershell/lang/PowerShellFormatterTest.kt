...
class PowerShellFormatterTest : FormatterTestCase() {
    ...
    private fun doTextTest(
        text: String,
        textAfter: String,
        reformatAsText: Boolean = true,
                ...
                try {
                    if (textRange != null) {
                        CodeStyleManager.getInstance(project)
                            .reformatText(file, textRange.startOffset, textRange.endOffset)
                    }
                    if (reformatAsText) {
                        val formatRange = file.textRange
                        CodeStyleManager.getInstance(project)
                            .reformatText(file, formatRange.startOffset, formatRange.endOffset)
                    } else {
    ...
    private fun createFileFromText(text: String) =
        PsiFileFactory.getInstance(project).createFileFromText(PowerShellLanguage.INSTANCE, text)

    private fun getPowerShellSettings(): PowerShellCodeStyleSettings =
        settings.getCustomSettings(PowerShellCodeStyleSettings::class.java)
