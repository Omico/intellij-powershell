...
class PowerShellCodeInsightTest : BasePlatformTestCase() {
    ...
    private val TEST_DATA_PATH =
        "src" + File.separatorChar + "test" + File.separatorChar + "resources" + File.separatorChar +
            "testData"
    ...
    private fun decodeVerbatimString(stringContent: String, expectedOutText: String) =
        checkStringDecode(stringContent, expectedOutText, false)

    private fun decodeVerbatimHereString(stringContent: String, expectedOutText: String) =
        checkStringDecode(stringContent, expectedOutText, false, true)

    private fun decodeExpandableHereString(stringContent: String, expectedOutText: String) =
        checkStringDecode(stringContent, expectedOutText, true, true)

    fun testDecodeVerbatimHereString() {
        decodeVerbatimHereString(
            "This is sample host \"string\" content",
            "This is sample host \"string\" content",
        )
        decodeVerbatimHereString(
...
//                      "This is a sample `\"string`\" with \$( Write-Output \"Sub-expression\" )end \$( Write-Output \"Sub-expression \" ) `\"of`\" string.")
        ...
        checkInjectedText(
            "text2 \"start\" $( Write-Output \"Hello 'Power' \"Shell\"!\") text end \"Here\" \"new text\";",
            "text2 \"\"start\"\" `$( Write-Output \"\"Hello 'Power' \"\"Shell\"\"!\"\") text end \"\"Here\"\" \"\"new text\"\";",
            defaultText = "var foo = \"\"old text\"\";",
        )
    ...
    private fun updateExpandableHereString(newInjectedText: String, expectedStringContent: String) =
        checkInjectedText(newInjectedText, expectedStringContent, true, true)

    private fun updateVerbatimString(newInjectedText: String, expectedStringContent: String) =
        checkInjectedText(newInjectedText, expectedStringContent, false)

    private fun updateVerbatimHereString(newInjectedText: String, expectedStringContent: String) =
        checkInjectedText(newInjectedText, expectedStringContent, false, true)

    // external editor -> PowerShell string
    private fun checkInjectedText(
        newInjectedText: String,
        expectedStringContent: String,
        isExpandable: Boolean = true,
        isHere: Boolean = false,
        defaultText: String = "var foo = 1",
    ) {
        val string = createStringExpression(isHere, isExpandable, defaultText)
        WriteCommandAction.writeCommandAction(project, myFixture.file)
            .withName("Test checkInjectedText. New Text='$newInjectedText'").run<Throwable> {
                string.updateText(newInjectedText)
                val newString = PsiTreeUtil.findChildOfType(myFixture.file, PowerShellStringLiteralExpression::class.java)
                    ?: error("file text='${myFixture.file}'")
    ...
    private fun checkStringDecode(
        stringContent: String,
        expectedOutText: String,
        isExpandable: Boolean = true,
        isHere: Boolean = false,
    ) {
    ...
    private fun createStringExpression(
        isHere: Boolean,
    ...
    private fun createStringExpression(nodeText: String): PowerShellStringLiteralExpression {
        ...
        return PsiTreeUtil.findChildOfType(file, PowerShellStringLiteralExpression::class.java)
            ?: error("text='$nodeText'")
    }
}
